...
import * as roomVideo from './rooms/Video.js';
...
const targetPositions = {
  hall: {
    ...
    models: new THREE.Vector3(0, 0, 0)
  },
  photogrammetry: {
...
export function init() {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    controllers[i].raycaster.far = 10;
  ...
  for (let i = 0;i < 2; i++) {
    ...
    controller.addEventListener( 'connected', function ( event ) {
      this.add(model.clone());
        ...
        if (event.data.handedness === handedness) gamepad = event.data.gamepad;
