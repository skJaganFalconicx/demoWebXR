...
export function setup(ctx) {
  ...
  scene.rotation.y = -Math.PI / 2;
    ...
    // for Astronaut
    var objParent = scene.getObjectByName('Animation').children[0].children[0].children[0].children[0].children[0];
    objParent.children[4].material.map = assets['Astronaut2_map'];
    objParent.children[4].material.aoMap = assets['Astronaut2_aO'];
    objParent.children[5].material.map = assets['Astronaut1_map'];
    objParent.children[5].material.aoMap = assets['Astronaut1_aO'];
    objParent.children[6].material.map = assets['Astronaut1_map'];
    objParent.children[6].material.aoMap = assets['Astronaut1_aO'];

    // for GSLV
    objParent = scene.getObjectByName('GSLV').children[0].children[0];
    for (var i = 0; i < objParent.children.length; i++) {
        objParent.children[i].children[0].material.map = assets[`GSLV${i}_map`];
        objParent.children[i].children[0].material.roughnessMap = assets[`GSLV${i}_metallness`];
        objParent.children[i].children[0].material.metalnessMap = assets[`GSLV${i}_metallness`];
        objParent.children[i].children[0].material.normalMap = assets[`GSLV${i}_normal`];
    }

    // for Satellite
    objParent = scene.getObjectByName('Satellite001').children[0].children[0];
    objParent.children[0].children[0].material.normalMap = assets['Satellite_commonNormal'];
    objParent.children[1].children[0].material.normalMap = assets['Satellite_commonNormal'];
    objParent.children[4].children[0].material.map = assets['Satellite_map'];
    objParent.children[4].children[0].material.metalnessMap = assets['Satellite_metallness'];
    objParent.children[4].children[0].material.roughnessMap = assets['Satellite_metallness'];
    objParent.children[4].children[0].material.normalMap = assets['Satellite_normal'];
