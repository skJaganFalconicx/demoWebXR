...
export function setup(ctx) {
  ...
  scene.rotation.y = -Math.PI / 2;
    ...
    const objParent = scene.getObjectByName('Sketchfab_model').children[0];
    objParent.children[0].material =
        new THREE.MeshBasicMaterial({ map: assets['pg_object0_tex'] });
    objParent.children[1].material =
        new THREE.MeshBasicMaterial({ map: assets['pg_object1_tex'] });
    objParent.children[2].material =
        new THREE.MeshBasicMaterial({ map: assets['pg_object2_tex'] });
    objParent.children[3].material =
        new THREE.MeshBasicMaterial({ map: assets['pg_object3_tex'] });
